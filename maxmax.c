...
int main()
{
    int m,n,A[10][10],max1,max2,vm_selected,vm_num,task_selected,sch[10][2],k = 0,total_time = 0;
  
    //m -> number of vms
    //n -> number of tasks
    //max1 -> stores the maximum duration for a particular task
    //max2 -> stores the maximum duration out of selected maximum 
    ...
    while(k < n)
    {
        ...
        for(int i=0;i<n; i++)
        {
            if(A[i][vm_selected] != -1)
            {
                A[i][vm_selected] += max2;
            }
        }
    }
    printf("\nTask\t\tMachine\n");
